...
func main() {
	...
	pg, err := service.ConnectPG(ctx, fmt.Sprintf(`postgres://postgres:%s@20.13.34.93:2345/openedub?pool_max_conns=10`,
		ev.Pgpass), log, "project_wikisearch_stage")
	...
	go pg.WriteToPG(ctx, "project_techblog_stage", data, errorChan, done)*/
	...
	log.Info("inserting to pg")
	go pg.WriteToPG(ctx, "project_wikisearch_stage", data, errorChan, done)

	<-done
	if err := pg.UpdateMainTable(ctx, "project_wikisearch_stage"); err != nil {
